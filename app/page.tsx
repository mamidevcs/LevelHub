...
function TopicList({
  ...
  type: "latest" | "popular" | "controversial";
  title: string;
  icon: JSX.Element;
  color: string;
}) {
  ...
  async function loadTopics(reset = false) {
    ...
    const res = await fetch(
      `/api/topics?type=${type}&skip=${reset ? 0 : skip}&take=${take}`
    );
  ...
  return (
    <section className="mb-12">
      <h2 className="text-xl font-bold mb-6 flex items-center">
        <span
          className="w-2 h-6 mr-2 rounded"
      ...
      {hasMore && !loading && (
        <div className="flex justify-center">
          <button
            onClick={() => loadTopics()}
            className="mt-4 px-40 py-2 rounded text-black font-semibold hover:brightness-90"
            style={{ backgroundColor: color }}
          >
            Daha fazla göster
          </button>
