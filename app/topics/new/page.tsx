'use client'; // React client component olarak çalıştırılır
import { useEffect, useState } from "react";
import Navbar from "@/components/Navbar";
import { FaHeading, FaAlignLeft, FaPlus } from "react-icons/fa";

export default function NewTopicPage() {
  const [title, setTitle] = useState<string>(''); // Başlık state'i
  const [content, setContent] = useState<string>(''); // İçerik state'i
  const [isLoading, setIsLoading] = useState(false); // Yükleniyor durumu
  const [message, setMessage] = useState<{text: string, type: 'success' | 'error'} | null>(null); // Başarı/hata mesajı
  const [tags, setTags] = useState<{id: number, name: string}[]>([]); // Etiket listesi
  const [selectedTagIds, setSelectedTagIds] = useState<number[]>([]); // Seçili etiket ID'leri

  // Sayfa yüklendiğinde etiketleri API'den çek
  useEffect(() => {
    fetch('/api/tags')
      .then(res => res.json())
  ...
  // Form gönderildiğinde yeni konuyu API'ye POST et
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setTitle(''); // Başlık temizle
        setContent(''); // İçerik temizle
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {tags.map(tag => (
                  <button
                    key={tag.id}
                    type="button" // Buton form submit yapmaz
                    onClick={(e) => {
                      e.preventDefault(); // Form submit engelle
                      toggleTag(tag.id); // Etiketi seç/kaldır
                    }}
