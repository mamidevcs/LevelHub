...
import { UserContext } from "../context/UserContext"; // Kullanıcı verisi ve çıkış fonksiyonu için context
import Link from "next/link";
import Navbar from "@/components/Navbar";

export default function ProfilePage() {
  const { user, logout, setUser } = useContext(UserContext); // Kullanıcı, çıkış ve setUser fonksiyonları
  ...
  useEffect(() => {
    if (user) {
      // Kullanıcı bilgilerini state'e koy
  ...
  if (!user) {
    // Giriş yapılmamışsa uyarı ve giriş linki göster
    return (
  ...
  async function handleUpdate(e) {
    ...
    try {
      ...
      // Başarılı mesaj ve kullanıcı bilgisini güncelle
